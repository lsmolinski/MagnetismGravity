...
The neutrino soliton serves as the reference wave centre for the BCC vacuum. Its geometric radius \(r_\nu\), derived in Section~\ref{sec:neutrino_anchor}, sets the fundamental length scale from which the EMC density hierarchy follows. The EMC spacing \(\lambda_l\) is the geometric Planck length derived in Section~\ref{sec:planck_from_geometry}.
This geometric saturation limit follows from the EWT granular density hierarchy developed by Yee \cite{yee2019spacetime}:
