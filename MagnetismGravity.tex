...
\begin{itemize}
	...
	\item \textbf{In the extreme EMC packaging density limit} (\(N \to N_{\nu,\text{max}}\)), the lattice becomes ultrastiff, and the propagation speed increases beyond \(c\). This regime corresponds to the early universe, where the causal structure differs from that of the vacuum.
\end{itemize}
...
\textbf{Remark on the independence of the metric conversion.}  
A possible concern is that Eq.~(\ref{eq:c_metric_definition}) merely replaces one constant, \(c\), with two others, \(\lambda_l\) and \(t_p\). This concern is unfounded. Both \(\lambda_l\) and \(t_p\) are derived from the same BCC geometry: \(\lambda_l\) from the packing fraction and the Eulerian dilution of the EMC lattice (Eqs.~(\ref{eq:lambda_l_from_Nnu}) and (\ref{eq:lambda_uncorr})), and \(t_p\) from the axiom of maximum propagation speed and the elastic response of that lattice. The ratio \(\lambda_l / t_p\) is therefore a structural invariant of the lattice, not a separate free parameter. The axiom provides the physical motivation: \(c\) is the maximum speed in the lattice at statutory density, and the ratio \(\lambda_l / t_p\) is its geometric expression.

Consequently, the EWT framework requires no empirical input for \(c\); it is a consequence of the discrete geometry and the statutory density \(N_{\nu,\text{stat}}\), analogous to the ratio of circumference to diameter (\(\pi\)) in Euclidean space.
