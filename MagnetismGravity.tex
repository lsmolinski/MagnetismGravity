...
The single remaining dimensional input is the electron mass 
$m_e$ --- the mass scale of the soliton --- which fixes the 
overall SI normalisation of $G$. In EWT, $m_e$ is itself 
derivable from $r_e$ and $c$ through the scaling law 
(Section~\ref{sec:numerical_verification_ewt}), so the 
independent primitive content of the $G$ identity reduces to 
one geometric length ($r_e$, determined by $K_{WC}=10$) 
