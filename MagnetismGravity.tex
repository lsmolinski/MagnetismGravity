...
A fundamental puzzle in any geometric theory of matter is how an asymmetric, rotating toroidal soliton (such as the electron) can generate a perfectly isotropic gravitational field. The resolution lies in a structural boundary layer that surrounds every soliton – the \textbf{Degraded EMC Wall}.
...
\end{equation}
where $r_{\text{wall}}$ denotes a point located strictly inside the Degraded EMC Wall. The exact value of this peak relative to $N_{\nu,\text{stat}}$ depends on the strength of the push-out – in ordinary solitons it remains moderate, while in extreme gravitational environments (such as those leading to a Geometric Black Hole) it can become very large.

The Degraded EMC Wall acts as a mandatory transducer between the toroidal core and the exterior. Because the individual EMCs are spherical units, the BCC lattice can only achieve a stable, static interface with the statutory background by adopting a spherical shape. The wall therefore enforces a \textbf{spherical boundary condition} on the energy density distribution $\rho_E(\theta,\phi)$ emerging from the toroidal core. The effect can be quantified by the \textit{Isotropy Operator} $\mathcal{I}$ (introduced in Sec.~\ref{sec:spherical_masking}). The Degraded EMC Wall thus decouples the internal toroidal dynamics from the external isotropic gravitational response.
...
\begin{itemize}
	...
	statutory value at infinity. In this case the wall represents 
...
with the only constraint that $\eta = \mathcal{O}(1)$.
...
\subsection{Isotropy and Spherical Masking: From Toroidal Core to Isotropic Gravity}
