...
($248.8$ ppm and $1177.2$ ppm, respectively) are internal EWT references, and 
the direct geometric derivation of the full, experimentally measured anomalous 
magnetic moments of the muon and tau from BCC lattice topology alone remains an 
open objective.
...
While the Enhanced EWT framework successfully exposes the semi-circular nature of QED precision in the electron sector -- where the fine-structure constant $\alpha$, itself an experimental input, is used to ``predict'' the very anomaly from which it is often extracted -- an analogous methodological caution applies to the present model. The current results for the muon and tau shell contributions ($248.8$ ppm and $1177.2$ ppm, respectively) are derived from internal EWT reference values obtained through the orbital mass relations (Section~\ref{sec:numerical_verification_ewt}). These targets are products of the same theoretical framework and serve as consistency benchmarks between the geometric and mass-generation sectors of EWT. Consequently, the agreement between the toroidal shell predictions and these internal references does not yet constitute a fully independent empirical prediction of the full anomalous magnetic moments $a_\mu$ and $a_\tau$ as measured by experiment. The direct geometric derivation of the complete, experimentally measured lepton anomalies from BCC lattice topology alone, without recourse to internally calibrated parameters, remains an open objective and a subject of ongoing investigation.
...
lattice. For the muon and tau generations, the model presently achieves high-precision 
internal consistency between its geometric and mass-generation sectors, while their 
fully independent empirical prediction remains an open objective. 
\newline
...
\begin{table}[ht]
	...
	\end{tabular}
		\vspace{5pt}
	\begin{flushleft}
		\footnotesize \textbf{Note on methodological status:} 
		In the EWT column, ``Yes'' for $a_{\mu}$ and $a_{\tau}$ denotes that the model 
		reproduces the corresponding internal shell-contribution targets with high precision, 
		as an internal consistency test between the geometric and orbital-mass sectors. 
		A fully independent, parameter-free derivation of the complete experimentally 
		measured $a_\mu$ and $a_\tau$ from BCC lattice topology alone remains an open objective.
    \end{flushleft}
\end{table}
