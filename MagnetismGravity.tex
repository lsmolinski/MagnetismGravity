...
\end{equation}
where \(X_{\text{eff}}\) is the geometric dilution factor defined in Section~\ref{sec:planck_from_geometry}.
...
Equation \eqref{eq:lambda_l_geometric} is therefore not an independent ansatz. It is the direct consequence of combining the geometric fine-structure constant, the geometric gravitational scaling, and the Planck-length definition into a single closed algebraic constraint. The \(e\) is Euler's number and \(r_\nu\) is the geometric neutrino radius derived in Section~\ref{sec:neutrino_anchor}.
