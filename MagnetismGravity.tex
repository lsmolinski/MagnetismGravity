...
neighbouring EMCs. In the continuum limit of the one-dimensional
lattice, the equilibrium spacing is
\(a \equiv 1/\eta\), where \(\eta\) is the normalised EMC density.
The stiffness is the second derivative of the pair potential evaluated
at that equilibrium separation:
\begin{equation}
	k(\eta)
	=
	\left.
	...
	.
\end{equation}
For a logarithmic repulsive core,
\(V(r) = -V_0 \ln(r)\), this gives
\begin{equation}
	k(\eta)
	=
	\frac{V_0}{(1/\eta)^{2}}
...
\begin{equation}
	k \;\propto\; \eta^{2}
	\label{eq:stiffness_scaling}
\end{equation}
is not an independent assumption. It follows from the same discrete
repulsion that underlies the Hookean description of the vacuum lattice.

The local metric phenomena considered earlier are encoded through two
relations:
\begin{equation}
...
\begin{equation}
	...
	.
\end{equation}
Both encodings can be recovered from the microscopic lattice dynamics
without postulating them.

Consider a one-dimensional spring-mass lattice. The mass per lattice
site is proportional to the local EMC density,
\begin{equation}
	m \;\propto\; \eta .
	\label{eq:mass_scaling}
\end{equation}
The local spring stiffness follows from
Eq.~\eqref{eq:stiffness_scaling}.

For small longitudinal disturbances, the speed of sound in this
lattice is
\begin{equation}
	v(\eta)
	=
...
\begin{equation}
	...
	,
\end{equation}
now obtained as an emergent property of the microscopic equations.

The corresponding effective refractive index follows from the same wave
speed:
\begin{equation}
	n_\gamma(\eta)
	=
	...
	.
\end{equation}
Thus the clock samples \(v(\eta)\), while the optical path samples
\(1/v(\eta)\). The square-root scaling emerges in both cases without
an independent optical postulate.

A local oscillator placed in a region of density \(\eta\) has the same
harmonic frequency scale,
\begin{equation}
	f
	\;\propto\;
	\sqrt{\frac{k}{m}}
...
Consequently, the two exponents previously introduced as geometric
encodings are the direct mechanical consequence of the lattice model.
They do not require an independent optical or clock hypothesis.
