...
\paragraph{Geometric Soliton Core ($\mathbf{4\pi^{3} + \pi^{2} + \pi}$):} 
...
\paragraph{Hypothesis of Spin Recovery Beyond the GBH Horizon}
...
\paragraph{Derivation of the Statutory Radius ($\mathbf{r_{\nu}}$):}
...
\paragraph{Calculation of the Statutory Constituent Count $\mathbf{N_{\nu}}$:}
...
\paragraph{Justification for the Geometric $\mathbf{g}$-Factor $\mathbf{g}_v$ and Consistency Check:}
...
The profound numerical relationship between the number of geometric constituents ($\mathbf{N_{\nu}}$) and the Gravitational Deficit ($\mathbf{\epsilon_{G}}$) must be framed as a \textbf{causal geometric necessity} derived from the EWT model's foundational principles, decisively countering the potential argument of numerology or parameter-fitting.

\paragraph{Physical Linkage:}
The Geometric Deficit ($\mathbf{\epsilon_{G}}$) is defined by the model as the scaling factor that mediates the fundamental \textbf{geometric scale} of the Elastic Medium Constituent ($\mathbf{EMC}$) to the global \textbf{geometric scale} of the entire Soliton structure. This scaling is physically manifested by the \textbf{total geometric volume} occupied by the Soliton's standing wave structure, which is quantified by $\mathbf{N_{\nu}}$.

Therefore, $\mathbf{1/\epsilon_{G}}$ is mathematically the \textbf{required total volume scaling factor} necessary to define the geometric configuration of the gravitational interaction. Since $\mathbf{N_{\nu}}$ explicitly counts this physical volume (ratio of $r_{\nu}^3$ to $l_{P}^3$), the scale of $\mathbf{N_{\nu}}$ establishes the \textbf{fundamental geometric scale} that the factor $\mathbf{1/\epsilon_{G}}$ \textbf{must embody}. This relationship is the \textbf{geometric definition} of the scale required to yield the gravitational interaction, where any numerical difference is attributed to dynamic effects.
...
\paragraph{The Base Gravitational Scaling ($\mathbf{G}_{\text{Base}}$)}
...
\paragraph{The $\hbar$-Independence of $\mathbf{G}_{\text{Base}}$}
...
\paragraph{Static Geometric Volume ($\mathbf{\pi^3}$)}
...
\paragraph{Potency 3: Dynamic Volume Scaling ($\mathbf{|\epsilon_M|}^{\mathbf{3}}$)}
...
\paragraph{Square Root: Gravitational Surface Effect ($\mathbf{\sqrt{\mathbf{N_{\nu, \text{effective}}}}})$}
...
\paragraph{Geometric Scaling Factor $\mathbf{N_{\nu, \text{effective}}}$}
...
\paragraph{Geometric Scaling Factor $\mathbf{A}_{\pi}$}
...
\paragraph{Role of the Dynamic Terms:}
...
\paragraph{Physical Justification: Gradient Packing Density}
...
\paragraph{Kinematic Defect and Measurement Discrepancies}
...
\paragraph{Table of Exact Parameters}
The consistency of the geometric identity is verified using precise CODATA 2022 values and EWT parameters derived from the fine-structure constant.
...
where $\mathbf{A}_{\pi}$ is the core geometric factor (specifically, $\mathbf{A}_{\pi}^{-1} = 4\pi^3+\pi^2+\pi$ in the full expression) and $\mathbf{N_{\nu,\text{effective}}}$ is 
derived according to the volumetric integral defined in Section \ref{sec:rho_r}.

\paragraph{Properties of the Operator $\mathcal{U}$.}
\begin{itemize}
...
\paragraph{Methodological Framework: Nodal Metrics and Geometric Determinism}
...
\begin{equation}
	...
	K_{n} = K_{n-1} + \text{round}\left( 10^{n-1} \cdot 2\pi^2 \right)
\end{equation}
...
\paragraph{3. Generation 3: Tau Expansion ($n=3$)}
...
\paragraph{Physical Interpretation: Geometric Continuity}
...
\paragraph{Physical Interpretation: Geometric Transition from 2D to 3D}
...
\paragraph{The Interface Tension ($\delta$)}
...
\paragraph{The Geometric Necessity of Shell Formation} 
...
\paragraph{1. Toroidal Geometry as a Structural Equilibrium}
...
\paragraph{2. Fibonacci Invariants ($L_{dim}$) as Lattice Latches}
...
\paragraph{3. AMM as a Manifestation of Vacuum Elasticity}
...
\paragraph{4. Structural Impedance and Dimensional Transition ($B_{\mu}$ vs $B_{\tau}$)}
...
\begin{table}[ht!]
	...
	\end{tabular}
\end{table}
\paragraph{Robustness of Recursive Convergence}
...
\paragraph{Geometric Permeability: The Missing Structural Link}
...
\paragraph{GW Speed and Dispersion.}
...
\paragraph{Vacuum Sensitivity and Stability Gain}
...
\paragraph{The Physical Constraint of $G$}
...
\paragraph{The Statutory Limit and the Threshold of Anti-Gravity}
...
\paragraph{Astrophysical Implications: The EMC Density Wall}
The validity of this \textbf{13.738\% structural margin} is supported by the dynamics of Gravitational Black Holes (GBH). In extreme gravitational collapse, the density deficit within the core necessitates a reciprocal \textbf{accumulation of the vacuum structural bond} at the event horizon’s boundary. This localized "EMC Density Wall" creates a region of extreme nodal stiffness, which anchors ultra-strong magnetic fields. Relativistic jets are thus interpreted as a \textbf{mechanical discharge mechanism}, preventing the local vacuum from exceeding the statutory density limit by releasing excess lattice potential.

\paragraph{The Event Horizon as a Gravitational Inversion Zone}
If the EMC accumulation at the event horizon exceeds the \textbf{Statutory Limit}, the local density gradient must undergo a sign reversal. In this scenario, the region immediately surrounding a GBH functions as a \textbf{source of localized anti-gravity}. This mechanical spiegeling of the gravitational vector explains the extreme acceleration of relativistic jets: they are not merely "escaping" a gravitational well, but are being \textbf{actively repelled} by the super-statutory density wall. This mechanism positions the Event Horizon not as a mathematical singularity, but as a physical phase-transition boundary between attractive deficit and repulsive saturation.
...
\paragraph{Dimensional Impedance and Slope Analysis}
...
\paragraph{Unified Stability Gain: The Global Nodal Anchor}
...
\begin{equation}
	|\epsilon_M| \implies 
	\begin{cases} 
		G = G_{Base} \cdot |\epsilon_M|^3 \cdot N_{\nu}^{-1/2} & \text{(Gravitational Coupling)} \\
		\alpha^{-1} = (4\pi^3 + \pi^2 + \pi) - |\epsilon_M| & \text{(Electromagnetic Scaling)} \\
		a_e = \frac{\alpha}{2\pi} \cdot \left( 1 - |\epsilon_M| \cdot \pi^3 \right) & \text{(Electron AMM Geometry)} \\
		a_l = \text{recursive}(|\epsilon_M|, L_{Fib}) & \text{(Leptonic Shell Resonance)}
	\end{cases}
\end{equation}

\paragraph{The Unitary Geometric Path}
...
\paragraph{Packing Fraction and the Vacuum Void}
...
\paragraph{Physical Justification: Sphericity as the Fundamental Unit}
...
\paragraph{Implications for Particle Topology}
...
\paragraph{Structural Justification: EMC Push-Out and Non-Uniform Density $\mathbf{\rho(r)}$}
...
\paragraph{Kinematic Defect and Model Consistency}
...
\paragraph{Local Repulsion vs. Global Attraction} 
...
\paragraph{Dual Action of the Push Out Operator:}
...
\paragraph{Unifying Role of Magnetic-Geometric Coupling}
...
\paragraph{Stability Invariance: The Nodal Anchor}
...
\paragraph{Recursive Determinism and the Unified Identity}
...
\paragraph{Numerical Verification of the Hierarchy}
...
\paragraph{Stability of the Recursive Chain}
...
\begin{itemize}
	...
	\begin{enumerate}
		\item \textbf{Static Geometric Base Factor ($\mathbf{A}_{\pi}$):} This is the constant scaling factor $\mathbf{\frac{1}{4\pi^{3} + \pi^{2} + \pi}}$, which is derived from the Soliton's fundamental geometry (e.g., its wavelength-to-radius ratio).
...
\begin{figure}[h!]
	...
	\begin{tikzpicture}[scale=1.7] % Zwiększono skalę, aby diagram był czytelny po zbliżeniu
		...
		\node[box, fill=red!10!white, draw=red!70!black] (Emergence) at (0, 0.5) {
			...
			($\mathbf{\Omega_{E}}$) \footnotesize $\propto \mathbf{A}_{\pi} \cdot \frac{1}{\mathbf{\sqrt{\mathbf{N_{\nu, \text{effective}}}}}}$
		};
...
\paragraph{A Philosophical Note: The Potential Hierarchy of Scales}
...
\subsection{The Geometric Nature of Gravitational Collapse: From Neutrinos to Black Holes}
\label{sec:gbh_neutrinos}

The non-linear scaling disparity established in Eq. \ref{eq:energy_scaling_boxed}--\ref{eq:geometry_scaling_boxed} leads to a profound reinterpretation of gravitational collapse. In the EWT framework, the distinction between leptonic matter and "dark" gravitational states (neutrinos and black holes) is defined by the presence or absence of magnetic spin compensation.
...
\begin{itemize}
	...
	\item \textbf{Geometric Black Holes (GBH)} emerge as massive clusters of such spin-less, collapsed solitons. In this state, the absence of magnetic "internal pressure" allows the geometric push-out deficit to reach its \textbf{absolute, yet finite saturation limit}.
\end{itemize}
...
\begin{enumerate}
	...
	\item \textbf{Electromagnetism:} It is crucial for the calculation and geometric correction of the \textbf{Fine-Structure Constant ($\alpha$)} (Eq. \ref{eq:alpha_full_geometric_deficit}), bridging the gap between electromagnetic coupling and soliton geometry.
...
\paragraph{Definition of Frameworks and Parameters}
...
\paragraph{Discussion of Predictive Autonomy}
...
\begin{figure}[h!]
	...
	\begin{tikzpicture}[
		...
		\node (n6) [block, below=of n5] {\textbf{6. THE COSMIC ARCHITECTURE} \\ \textbf{Black Holes (GBH):} Minimal Packing Limit of the universal structural bond \\ \textbf{Astrophysical Jets:} Mechanical discharge of the redistributed EMC density wall};
...
\appendix
\section{List of Model Parameters and Physical Constants}
...
\begin{table}[h!]
	...
	\begin{tabularx}{\textwidth}{p{4.0cm} p{1.8cm} X p{4.0cm}}
		...
		\midrule
		Geometric $\mathbf{A}_{\pi}$ Base & $\mathbf{A}_{\pi}^{-1}$ & $4\pi^{3} + \pi^{2} + \pi$ & EWT Static Geometric Foundation \\
		...
		Emergent Base Factor & $\mathbf{\Omega_{E}}$ & $\mathbf{A}_{\pi} \cdot \mathbf{N_{\nu, \text{effective}}}^{-1/2}$ & Static Geometry Composite \\
...
\section{Numerical Verification Script Output}
...
\section{EWT vs Standard Model – Quantitative Precision Comparison}
\label{app:EWT_VS_SM}
...
\section{EWT vs Standard Model – Quantitative Precision Comparison Output}
...
\section{Leptonic Resonance Mapping (AMM Search)}
\label{app:AMM_find}
