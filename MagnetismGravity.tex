...
\begin{table}[ht]
	...
	\begin{tabular}{lccc}
		\toprule
		\textbf{Generation} & \textbf{Target} & \textbf{EWT Prediction} & \textbf{Relative Error} \\
		\midrule
	...
	\end{tabular}
\begin{flushleft}
	\small \textit{Note: The electron target is the full CODATA $(g-2)/2$ value. 
		The muon and tau targets are EWT internal shell references derived in-script 
		from the orbital mass relations; their explicit numerical derivation is 
		provided in Listing~\ref{lst:EWT_VS_SM}.}
\end{flushleft}
\end{table}
...
Section~\ref{sec:numerical_verification_ewt} (see 
Table~\ref{tab:mass_precision_comparison}) achieves sub-percent mass precision 
but presently employs internal calibration factors derived from the electron 
rest mass. Consequently, the shell contribution targets for the muon and tau 
($248.8$ ppm and $1177.2$ ppm, respectively) are internal EWT references, 
while the anomalous magnetic moments themselves are genuine predictions of the 
BCC lattice geometry. The simultaneous, fully parameter-free derivation of both 
mass and AMM for all three generations from BCC lattice topology alone remains 
an open objective.
