...
BCC lattice geometry and the topological class of the electron.
...
The simplest form compatible with the NLS-type soliton equations is the cubic
...
characterised by an integer winding number
\begin{equation}
	Q = \frac{1}{4\pi} \int_{S^{2}} \Psi^{*}\,\omega \in \mathbb{Z},
\end{equation}
...
Equation~\eqref{eq:soliton_wave_eq} together with the nonlinear term
\eqref{eq:F_cubic} (or \eqref{eq:F_density}) and the topological condition
$Q=10$ constitutes a complete, falsifiable model of the electron core:
...
\begin{enumerate}
	...
	limit $8\pi^{7}$) fixes the numerical value of the coupling without
...
\begin{itemize}
	...
	\item Two candidate forms for $\mathcal{F}$ are proposed:
	$\gamma\Psi^{3}$ (pure cubic) and $\gamma(1-\rho/\rho_{0})\Psi^{3}$
	(density-modulated).
