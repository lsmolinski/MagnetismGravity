...
this stability is described in Section~\ref{sec:nonlinear_stability}, where
...
self-consistent solution that simultaneously satisfies:
...
represents the lowest-energy, stable homotopy class capable of establishing a
fully isotropic boundary condition on $S^2$ (analogous to the global minimum of a
spherical Thomson packing problem for discrete phase nodes).
...
variants and the topological condition $Q=10$ constitutes a complete,
falsifiable model of the electron core:
...
\end{equation}
Because both $\lambda_C$ and $\alpha$ follow from the same BCC lattice
geometry, $r_{\text{wall}}$ is not a new free parameter but a structural
consequence of the vacuum topology.  This links the nonlinear stabilisation
mechanism directly to the Compton annihilation threshold: the same geometric
framework that sets the electron's mass, radius, and magnetic anomaly also
determines the size of the region within which the nonlinearity is active,
providing a unified description of particle stability, forces, and
annihilation. The considerable width of the wall --- roughly $860\,r_e$ --- is not an
arbitrary feature but a geometric consequence of the ratio $1/\alpha
\approx 137$.  Physically, this extended transition zone provides a
broad buffer within which the nonlinearity can adiabatically relax the
EMC density from its deep core deficit to the statutory background,
suppressing the sharp gradients that would otherwise destabilise the
standing wave.
...
\begin{itemize}
	...
	is a geometric consequence of the Compton wavelength and ensures a broad
