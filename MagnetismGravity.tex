...
We posit the following fundamental axiom:
...
A possible concern is that Eq.~(\ref{eq:c_metric_definition}) merely replaces one constant, \(c\), with two others, \(\lambda_l\) and \(t_p\). This concern is unfounded. Both \(\lambda_l\) and \(t_p\) are derived from the same BCC geometry: \(\lambda_l\) from the packing fraction and the Eulerian dilution of the EMC lattice (Eqs.~(\ref{eq:lambda_l_from_Nnu}) and (\ref{eq:lambda_uncorr})), and \(t_p\) from the elastic response of that lattice. The ratio \(\lambda_l / t_p\) is therefore a structural invariant of the lattice, not a separate free parameter. The axiom provides the physical motivation: \(c\) is the maximum speed in the lattice at statutory density, and the ratio \(\lambda_l / t_p\) is its geometric expression.
