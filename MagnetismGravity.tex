...
\textbf{Remark on time dilation:} The oscillation period of the
soliton, as measured by a rest-frame observer, dilates to
$T' = \gamma T_0$, so the observed tick rate of a co-moving clock
scales as $f' = f_0/\gamma$ --- the temporal counterpart of the
spatial contraction derived above. In the natural units of the
BCC lattice ($c = 1$, $[m] = [s]$), this follows directly from the
requirement that the standing-wave soliton maintain a constant
spatial-to-temporal aspect ratio in its rest frame.
