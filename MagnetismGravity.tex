...
\begin{enumerate}
	...
	\end{equation}
	where $\chi>0$ is the effective mobility coefficient of the BCC lattice. The negative sign signifies structural displacement \emph{toward} the central deficit. Light rays propagating through this strained lattice continuously realign along the principal axes of the displacement field, deflecting the wavefront toward the mass center.
\end{enumerate}

\subsection{Bridging the Vector Displacement to the Scalar Refractive Index}
\label{sec:bridging_u_to_n}

To connect the vector kinematics of Eq.~\eqref{eq:emc_displacement_vector} to standard ray-tracing integrals, we express the scalar index $n(r)$ as the integrated trace of the radial displacement field $u_r(r) = |\vec{u}(r)|$.

A 3D standing-wave soliton exhibits an asymptotic radial energy density deficit decaying as $1/r$. In the simplest spherical weak-field model, the local EMC density takes the form
\begin{equation}
	N_{\nu}(r) = N_{\text{stat}}\left(1 - \frac{2r_s}{r}\right),
\end{equation}
so that the static EMC density gradient scales as
\begin{equation}
	\left|\nabla N_{\nu}(r)\right| \propto \frac{r_s}{r^{2}},
\end{equation}
where $r_s = 2GM$ is the gravitational radius in natural units ($c = 1$). Integrating this gradient along the radial trajectory yields a cumulative radial displacement magnitude:
\begin{equation}
	u_r(r) = \int_{r}^{\infty} \left|\nabla N_{\nu}(r')\right|\, dr' \;\propto\; \frac{r_s}{r}.
