...
The neutrino, \textbf{lacking magnetic coherence}, cannot enter this compensatory regime, which explains its \textbf{larger equilibrium radius} ($r_{\nu}$) as derived in Section \ref{neutrion_radius_interaction}. While the charged leptons (electron, muon, tau) are "compressed" by their intrinsic magnetic/spin energy, the neutrino remains in a relaxed geometric state, defining the statutory volume deficit ($N_{\nu, \text{stat}} \approx 10^{52}$) used to calculate the base Gravitational Constant $G$.
...
In the EWT framework, following the geometric interpretation proposed by Yee and Gardi \cite{yee2019geometry}, antimatter is defined as a soliton state characterized by a $180^\circ$ ($\pi$) phase-shift relative to the primary BCC lattice nodes. The observable dominance of matter is reinterpreted as a result of \textbf{Non-linear Phase Conversion} occurring at the \textbf{trans-statutory boundary} ($N > N_{\nu, \text{stat}}$) of EMC Walls.
...
\begin{itemize}
	\item \textbf{Nodal Stiffness ($N$) as a Vacuum Modulus:} Unlike the discrete node count ($K_{WC}$), the parameter $N$ represents the \textbf{mechanical resilience} of the vacuum structural bond. It is the "Quantum Young's Modulus" that determines how much the lattice resists the $r^5$ energy surge of the soliton.
