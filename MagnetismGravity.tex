...
undergo a non‑symmetrical Doppler shift.  To maintain the stability of the
wave‑center nodes, the geometry of the standing wave must adjust.
...
``size'' of the particle --- must contract in the direction of motion to
preserve resonance within the BCC lattice.
...
= 2L_0\gamma$ for both arms.  Since the rest‑frame round‑trip time
is $t_0 = 2L_0$, a clock carried by the moving interferometer must
tick slower by precisely the factor $\gamma$ to measure the same
elapsed time.  The oscillation period of the soliton therefore
dilates to $T' = \gamma T_0$, with the observed tick rate scaling
as $f' = f_0/\gamma$ — the temporal counterpart of the spatial
...
\begin{center}
	\fbox{
		\begin{minipage}{0.85\textwidth}
			\centering
			\vspace{5pt}
			\small \textbf{Conclusion on Relativistic Covariance:} \\
			The Doppler‑based derivation of Lorentz contraction, due to
			LaFrenière~\cite{LaFreniere2000}, is embedded in the
			EWT framework: the abstract geometry of Minkowski
			space is replaced by the physical elasticity of the BCC
			lattice.  The compression of matter becomes the necessary
			condition for maintaining the nodal stiffness equilibrium
			($N_{\text{final}}$) of the standing‑wave soliton at high
			velocities.
			\vspace{5pt}
		\end{minipage}
	}
...
$L_{dim} \in \{5, 34\}$.  To maintain notational precision, a distinction is
...
\begin{equation}
	...
	\qquad
	M_{\tau}^{\mathrm{ideal}}
	= 1 + 22\pi^{2} \approx 218.131.
\end{equation}

In the ideal continuous limit, omitting the rounding in Eq.~\eqref{eq:K_recursive_final}, 
one obtains \(K_{\tau}^{\text{ideal}} = 10 + 110 \cdot 2\pi^{2}\), and hence 
