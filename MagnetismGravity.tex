...
\begin{equation}\label{eq:epsM_stability}
	...
	= \frac{1}{8\pi^{7}} .
\end{equation}
Physically, $\epsilon_{M}$ measures the fractional loss of stiffness caused by
the soliton's spin-induced magnetic torque. To eliminate any systemic ambiguity
with the standard wave vector $k$, the natural nonlinear coupling strength is
designated by the parameter $\gamma$, defined as the inverse of this deficit:
...
prove decisive for discriminating $K_{WC}=10$ from neighbouring configurations in
...
\begin{enumerate}
	...
	a unique axis (1 or 2) or introduce internal phase frustration (4 or more on
	a single shell at this radius).
	...
	positions saturate the remaining propagation axes, guaranteeing isotropic
	far-field emission after spherical masking by the Degraded EMC Wall
	(Section~\ref{sec:spherical_masking}).
\end{enumerate}

A configuration such as 2-3-5 or 2-2-6 would either misalign the spin axis
or leave some BCC axes unmatched, producing a net multipole moment that the
nonlinearity cannot cancel.  The 1-3-6 triad is therefore the unique
self-consistent candidate that simultaneously satisfies:
...
\begin{enumerate}
	...
	resists dispersion.
	\item The \textbf{topological winding number} guarantees that the ground state
	has exactly ten wave centres --- not by empirical input, but as a consequence
	of the homotopy class of the field on $S^{2}$ under $Im\bar{3}m$ constraints.
...
\begin{equation}
	\gamma = \frac{1}{\epsilon_{M}} = 8\pi^{7}
	= 8\;\cdot\;\pi^{(1)}\pi^{(2)}\pi^{(3)}\pi^{(4)}\pi^{(5)}\pi^{(6)}\pi^{(7)},
\end{equation}
where the seven factors correspond to the seven degrees of freedom of the
charged weak scale (Table~\ref{tab:dimensional_ladder_origin}). Thus the
stabilisation of the electron is not an isolated electromagnetic phenomenon
but a direct mechanical consequence of the full 7‑dimensional phase space
that the BCC lattice makes available to a charged soliton. The factor~8
reflects the eight primary propagation axes of the $Im\bar{3}m$ unit cell,
ensuring that the nonlinear restoring force acts isotropically.
...
Numerically $\lambda_C \approx 2.426 \times 10^{-12}$~m, which is roughly
$860\,r_e$ and therefore lies well outside the soliton core.  The EMC
density $\rho(r)$ must recover its statutory background value $\rho_0$
somewhere between $r_e$ and $\lambda_C$.  A plausible scale is
\begin{equation}
...
width of the wall --- roughly $860\,r_e$ --- is not an arbitrary feature but a
...
\begin{itemize}
	...
	\end{enumerate}
	\item The Degraded EMC Wall provides the necessary boundary condition
	for the nonlinearity: its radius, estimated as $r_{\text{wall}} \sim r_e / \alpha$,
	is consistent with the Compton wavelength and ensures a broad
	adiabatic transition zone ($\sim 860\,r_e$) in which the density-modulated
	term can act without sharp gradients that would otherwise destabilise the
	standing wave.
	\item The winding number $Q=10$ on $S^{2}$ provides the topological
	selection rule that isolates the electron ground state under $Im\bar{3}m$ constraints.
	\item The 1-3-6 partition is the unique arrangement that simultaneously satisfies
	the topological winding, the cubic nonlinearity, and the octahedral symmetry
	of the BCC lattice.
	\item The energetic ($r^{5}/r^{3}$) and topological arguments are complementary:
	the former restricts the range of allowed $K$, the latter restricts $K_{WC}$ to
	integer values; together they single out $K_{WC}=10$.
	\item The coupling $\gamma = 8\pi^{7}$ directly expresses the 7‑dimensional
	charged-weak budget of the BCC lattice.
