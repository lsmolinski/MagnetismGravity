...
To connect the vector kinematics of Eq.~\eqref{eq:emc_displacement_vector} to standard ray-tracing integrals, we evaluate the scalar refractive index \(n(r)\) as the local isotropic dilatation factor induced by the dimensionless radial displacement field \(u_r(r)\).
...
The scalar refractive index \(n(r)\) represents the local isotropic dilatation factor induced by this displacement field:
\begin{equation}
	n(r) = 1 + u_r(r) = 1 + \frac{r_s}{r} = 1 + 2\Phi_N(r),
...
\begin{equation}
	n(r) = \left(1 - \frac{2r_s}{r}\right)^{-1/2}.
	\label{eq:n_full}
\end{equation}
...
Formally, \(n(r)\) matches the spatial metric component \(g_{rr}^{-1/2}\) of the Schwarzschild solution. In EWT, however, this exact mathematical form arises physically from the nonlinear elastic response of the EMC medium under the \(1/r\) gravitational potential load, without requiring a curved spacetime manifold.

Expanding Eq.~\eqref{eq:n_full} binomially:
\begin{equation}
	n(r) = \left(1 - \frac{2r_s}{r}\right)^{-1/2} \approx 1 + \frac{r_s}{r} + \mathcal{O}\left(\left(\frac{r_s}{r}\right)^2\right),
\end{equation}
recovers the exact factor of 2 in front of \(\Phi_N(r)\), providing the structural lattice foundation for the observed \(1.75''\) solar limb deflection. In the undisturbed statutory vacuum (\(r_s/r \to 0\)), \(n(r) \to 1\), restoring the fundamental lattice ratio \(c = \lambda_l / t_p = 1\).
...
\begin{itemize}
	...
	\item \textbf{Spatial metric component ($g_{rr}^{-1/2}$):}
