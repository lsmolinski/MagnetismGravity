...
In the EWT framework, mass is an emergent property of wave center density ($K$) within the aether. Numerical simulation categorizes mass generation into three distinct geometric modes, reflecting the diverse structural signatures of subatomic particles:
...
\begin{enumerate}
	...
	\item \textbf{Phase-Correction Mode (Quark Sub-shells):} Used for Quarks ($u, d, s$), adjusting the spherical resonance for off-axis shell placement through a correction factor $\epsilon_{qrk}$:
	\begin{equation}
		E_{qrk(K_{WC})} = E_{sph(K_{WC})} \cdot \epsilon_{qrk}
	\end{equation}
\end{enumerate}
...
The results of this numerical reproduction are summarized in Table \ref{tab:mass_precision_comparison}. The high fidelity of the model—specifically the sub-0.01\% error margins for the Muon, Tau, and Higgs boson—validates the use of discrete wave center counts as a viable alternative to the Higgs mechanism.
...
\subsection{Discussion on Boson Deviation}
While the majority of particles exhibit near-zero deviation, the 9.00\% error observed for the W Boson ($K_{WC}=109$) suggests a discrete phase-shift jump or a lattice-driven volume deficit not fully captured by the purely spherical $({K_{WC}})^5$ scaling. This deviation points to a potential structural complexity in charged heavy bosons, likely related to the symmetry breaking between the W and Z states, which remains a subject for further geometric refinement.
...
\begin{table}[h!]
	...
	\begin{tabular}{lcccc}
		\toprule
		\textbf{Constant} & \textbf{EWT Prediction} & \textbf{CODATA 2022} & \textbf{Error (ppm)} & \textbf{Error (\%)} \\
		\midrule
		$R_\infty$ (m$^{-1}$) & $10973670.62263460$ & $10973731.56815700$ & 5.55 & $5.55 \times 10^{-4}\%$ \\
		$a_0$ (m) & $5.291791330634349 \times 10^{-11}$ & $5.291772109030000 \times 10^{-11}$ & 3.63 & $3.63 \times 10^{-4}\%$ \\
		$\lambda_C$ (m) & $2.426314389900505 \times 10^{-12}$ & $2.426310238670000 \times 10^{-12}$ & 1.71 & $1.71 \times 10^{-4}\%$ \\
		\bottomrule
	\end{tabular}
\end{table}
