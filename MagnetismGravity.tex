...
\subsubsection{Physical Interpretation of Modulators: Reflection vs. Interference}
\label{sec:modulators_interpretation}

The distinction in the mathematical form of the modulators—linear for the bosonic sector ($C_{\text{gap}}$) and quadratic for the fermionic sector ($C_{\text{fermion}}$)—is not arbitrary. It originates from the fundamentally different nature of the excitations interacting with the BCC lattice.

\begin{itemize}
	\item \textbf{Vector Bosons (Volumetric Reflection):} 
	The $W$ and $Z$ bosons are treated as compact, volumetric solitons, engaging the full 6-dimensional interaction space ($\pi^6$) of the lattice. In this sector, the modulator $C_{\text{gap}} = (1 + \pi^6 C_{\text{local}})$ acts as a singular phase-shift correction. Physically, the boson, as a unified entity, "reflects" off the lattice impedance, encountering the structural resistance as a single, coherent object. This process results in a linear correction to its mass-energy attractor.
	
	\item \textbf{Fermions (Bi-local Interference):} 
	In contrast, the Cabibbo mixing angle describes the relationship between two distinct boundary solitons ($d$ and $s$ quarks). Here, the interaction is not a simple reflection, but a wave interference process between two states on the 5D phase-surface ($\pi^5$), which represents the projection of 3D momentum degrees of freedom onto the 2D spherical boundary. Since each of the two states independently interacts with the lattice impedance, the mixing amplitude is modulated twice—once for each node of the interaction.
	
	The quadratic term in $C_{\text{fermion}} = (1 + \pi^5 C_{\text{local}})^2$ mathematically represents this \textbf{bi-local interference} pattern. While a boson experiences a single scattering event, fermions undergo a joint interference process within the boundary layer, necessitating the squared modulator to account for the combined probability amplitude of the flavor-mixing transition. This quadratic scaling is consistent with the standard formulation of transition probabilities, where the mixing intensity is proportional to the square of the overlapping wave amplitudes, each independently shifted by the lattice's boundary impedance. The reduction from $\pi^6$ (bosonic volume) to $\pi^5$ (fermionic boundary) reflects the holographic principle within the EWT framework: heavy bosons occupy the full lattice bulk, while fermion mixing is restricted to the 5D manifold of the soliton's surface.
\end{itemize}
...
\begin{table}[ht]
	...
	\begin{tabular}{lllcl}
		\toprule
		\textbf{Scale} & \textbf{Interaction} & \textbf{Interaction Geometry} & \textbf{Coupling Factor} & \textbf{EWT Value} \\
		\midrule
		$\pi^7$ & Charged Weak & 3D Volumetric + 1D Amp. & Calibrated & $M_W$ Attractor \\
		$\pi^6$ & Neutral Weak & 3D Volumetric & $\pi^6 C_{\text{local}}$ & $1.4075 \times 10^{-2}$ \\
		$\pi^5$ & Flavor Mixing & 2D Surface (Holographic) & $\pi^5 C_{\text{local}}$ & $4.4804 \times 10^{-3}$ \\
