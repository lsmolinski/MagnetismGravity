...
\begin{table}[ht]
	...
	\begin{flushleft}
		...
		The only empirical input entering the muon and tau predictions is the mass 
		scale, fixed by the orbital mass relations 
		(Section~\ref{sec:numerical_verification_ewt}). The pure $K_{WC}^5$ 
		meson-mode (Section~\ref{sec:meson_mode_scan}) further delivers an 
		independent, parameter-free prediction of the muon and tau masses at the 
		$\sim 0.8\%$ level.
	\end{flushleft}
\end{table}
